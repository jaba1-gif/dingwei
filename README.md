...
## If another app still shows the old location

Try this checklist:

1. Open **Developer Options** and confirm **Select mock location app** is set to **Mock Location**.
2. In **Mock Location**, tap **Start**, wait 3 seconds, then tap **Check System Location**.
3. If the check says `mock=true`, Android accepted the mock location.
4. Force stop and reopen the other app. Some apps keep a cached location.
5. Make sure the other app has location permission enabled.
6. Turn off battery optimization for **Mock Location** if the service is stopped in the background.

If Android accepted `mock=true` but the other app still refuses the location, that app may be detecting mock locations or using its own anti-fraud location SDK. This project does not bypass those protections.
